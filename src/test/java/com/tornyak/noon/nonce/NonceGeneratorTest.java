...
public class NonceGeneratorTest {
	...
	public void generateNonce() {
		...
		assertThat(nonce.getBytes().length).isGreaterThanOrEqualTo(NonceTestConfig.NONCE_SIZE_BYTES);
	}

	@Test
	public void nonceUnique() {
		int expectedSize = 100;
		Set<Nonce> nonces = Stream.generate(nonceGenerator).limit(expectedSize).collect(Collectors.toSet());
